...
for k = 1:num_labels
    y_col=y_matrix(:,k);
    a3_col=a3(:,k);
    temp = temp + -y_col'*log(a3_col) - (1 - y_col') * (log(1-a3_col));
end;
%h = sigmoid(X*theta);
%r = lambda * sum(theta(2:end).^2) / (2 * m);
theta1_r=Theta1(:,2:end).^2;
theta2_r=Theta2(:,2:end).^2;
r_temp1=sum(theta1_r(:));
r_temp2=sum(theta2_r(:));
r = (lambda * (r_temp1+r_temp2)) / (2 * m);
...
temp_col=Theta2(:,2:end);
sigGrad=sigmoidGradient(z2);
d2=d3*temp_col.*sigGrad;
